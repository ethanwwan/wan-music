...
def extract_kuwo_playlist(raw: dict, **kwargs) -> dict:
    ...
    接口：https://m.kuwo.cn/newh5app/wapi/api/www/playlist/playListInfo?pid={pid}&pn={page}&rn=100
    ...
    返回：{
        ...
        'cover': str,        # 封面 URL
        'trackCount': int,   # 总歌曲数
        'tracks': [song, ...]  # 歌曲列表（已标准化的 dict）
    }
    """
    data = raw.get('data') or {}
    name = data.get('name') or ''
    creator = data.get('creator') or data.get('userName') or ''
    cover = data.get('img') or data.get('pic') or data.get('cover') or ''
    if cover and not cover.startswith('http'):
        cover = 'https:' + cover if cover.startswith('//') else 'https://img1.kuwo.cn/playlistcover/' + cover
    ...
    for t in raw_tracks:
        if not isinstance(t, dict):
            continue
        # 复用 search 链的 normalize 逻辑，确保字段一致
        try:
...
def extract_gdstudio_playlist(raw: dict, **kwargs) -> dict:
    ...
    接口：https://music-api.gdstudio.xyz/api.php?types=playlist&id={id}&source=...

    注意：gdstudio 的 source 参数**实际无效**，无论传什么都返回网易云格式
    （顶层 playlist 节点，tracks 字段为网易云歌单标准格式：ar[] / al{} / h/m/sq/hr/fee）

    策略：解析网易云歌单格式的响应，按 kwargs 中的 target_platform 标记每首 track 的 source
    """
    if not isinstance(raw, dict) or int(raw.get('code', 0)) != 200:
        return {'name': '', 'creator': '', 'cover': '', 'trackCount': 0, 'tracks': []}

    # gdstudio 顶层固定是 playlist（不是 data）
    ...
    # 即使 gdstudio 实际是网易云数据，我们仍按用户原始 URL 标记 source
    # 但 qualityMap 不能从网易云格式直接映射到酷我音质，所以 source 仍标记为 netease
    # （播放时仍可走 gdstudio 拿 URL）
    ...
    for t in raw_tracks:
        if not isinstance(t, dict):
            continue
        try:
            # 复用 normalize_netease_song 标准化（已知格式：ar[]/al/fee/h/m/l/sq/hr/dt）
            normalized = normalize_netease_song(t)
            normalized['source'] = target_platform  # 用户原始 URL 的平台
