...
| 环境 | 端口 | 配置 |
|------|------|------|
| 开发（dev:api） | 5005 | `frontend/.env.dev` |
| 生产预览（preview:api） | 6005 | `WAN_MUSIC_ENV_FILE=frontend/.env.prod` |
| Docker 部署 | 5002 | `.env.docker` 的 `CONTAINER_PORT` |

### 环境变量优先级

```
WAN_MUSIC_ENV_FILE > frontend/.env.dev > .env.docker
```

- `dev:api` 启动时用默认 `.env.dev`（5005）
- `preview:api` 启动时显式设置 `WAN_MUSIC_ENV_FILE=frontend/.env.prod`（6005）
- Docker 部署用 `.env.docker`（5002）
